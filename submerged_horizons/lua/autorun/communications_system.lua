...
if SERVER then
    ...
    util.AddNetworkString("comms_open_ship_panel")
    util.AddNetworkString("comms_hail_ship")
    util.AddNetworkString("comms_sync_ships")
    util.AddNetworkString("comms_add_ship")
    util.AddNetworkString("comms_remove_ship")
end
...
SHRPComms.Ships = SHRPComms.Ships or {}
...
function SHRPComms.CreateShipCall(initiator, participants)
    if not IsValid(initiator) or not participants or #participants < 1 then
        return nil
    end
...
if SERVER then
    ...
    net.Receive("comms_hail_ship", function(len, ply)
        local shipName = net.ReadString()
        local ship = SHRPComms.Ships[shipName]
        if not ship then
            ply:ChatPrint("Ship not found.")
            return
        end
        ...
        for _, p in ipairs(player.GetAll()) do
            if IsValid(p) and p:GetPos():Distance(ship.pos) <= (ship.range or 1000) then
                table.insert(participants, p)
            end
        end

        local call = SHRPComms.CreateShipCall(ply, participants)
        if call then
            ply:ChatPrint("Hailing " .. shipName .. " with " .. #participants .. " personnel.")
        end
    end)

    net.Receive("comms_add_ship", function(len, ply)
        local name = net.ReadString()
        local pos = net.ReadVector()
        if name and name ~= "" and pos then
            SHRPComms.Ships[name] = {pos = pos, range = 1000}
            net.Start("comms_sync_ships")
            net.WriteTable(SHRPComms.Ships)
            net.Broadcast()
        end
    end)

    net.Receive("comms_remove_ship", function(len, ply)
        local name = net.ReadString()
        if SHRPComms.Ships[name] then
            SHRPComms.Ships[name] = nil
            net.Start("comms_sync_ships")
            net.WriteTable(SHRPComms.Ships)
            net.Broadcast()
        end
    ...
    hook.Add("PlayerInitialSpawn", "SHRPCommsSyncShips", function(ply)
        net.Start("comms_sync_ships")
        net.WriteTable(SHRPComms.Ships)
        net.Send(ply)
    end)
...
if CLIENT then
    ...
    net.Receive("comms_sync_ships", function()
        SHRPComms.Ships = net.ReadTable()
    end)

    net.Receive("comms_open_ship_panel", function()
        local frame = vgui.Create("DFrame")
        frame:SetTitle("Select Ship to Hail")
        ...
        list:SetSize(280, 120)
        list:SetPos(10, 30)
        list:AddColumn("Ship Name")

        for name, _ in pairs(SHRPComms.Ships) do
            list:AddLine(name)
        end

        local hailBtn = vgui.Create("DButton", frame)
        hailBtn:SetPos(10, 160)
        hailBtn:SetSize(280, 30)
        hailBtn:SetText("Hail Selected Ship")
        hailBtn.DoClick = function()
            local line = list:GetSelectedLine()
            if line then
                local name = list:GetLine(line):GetValue(1)
                net.Start("comms_hail_ship")
    ...
    spawnmenu.AddToolTab("SHRP Comms", "SHRP Comms", "icon16/comments.png")
    spawnmenu.AddToolMenuOption("SHRP Comms", "Manage", "Manage Ships", "Manage Ships", "", "", function(panel)
        panel:ClearControls()
        ...
        for name, data in pairs(SHRPComms.Ships) do
            list:AddLine(name, tostring(data.pos))
        end

        panel:AddItem(list)

        local addBtn = vgui.Create("DButton", panel)
        addBtn:SetText("Add Ship")
        addBtn.DoClick = function()
            ...
            nameEntry:SetPlaceholderText("Ship Name")

            local pos = LocalPlayer():GetPos()
            local posBtn = vgui.Create("DButton", frame)
            posBtn:SetPos(10, 60)
            posBtn:SetSize(280, 30)
            posBtn:SetText("Set Position to Current Location")
            posBtn.DoClick = function()
                pos = LocalPlayer():GetPos()
                posBtn:SetText("Position Set")
            end

            local saveBtn = vgui.Create("DButton", frame)
            saveBtn:SetPos(10, 100)
            saveBtn:SetSize(280, 30)
            saveBtn:SetText("Save")
            saveBtn.DoClick = function()
                local name = nameEntry:GetValue()
                if name and name ~= "" then
                    net.Start("comms_add_ship")
                    net.WriteString(name)
                    net.WriteVector(pos)
                    net.SendToServer()
                    list:AddLine(name, tostring(pos))
                    frame:Close()
                end
            end
        end
    ...
    net.Receive("comms_hail_started", function()
        ...
        elseif typ == "ship" then
            activeComm = {type = typ, label = "Ship-to-ship comms from " .. initiator}
            chat.AddText(Color(100, 255, 120), "Ship-to-ship communications established.")
        end
    end)
    ...
    net.Receive("comms_hail_ended", function()
        ...
        elseif typ == "ship" then
            chat.AddText(Color(255, 150, 150), "Ship-to-ship comms ended by " .. initiator .. ".")
        else
    ...
    hook.Add("Think", "SHRPCommsVoiceVolumeOverride", function()
        if activeComm then
            for _, ply in ipairs(player.GetAll()) do
    ...
    hook.Add("PlayerStartVoice", "SHRPCommsVoiceStart", function(ply)
        if activeComm then
            ply:SetVoiceVolumeScale(1)
        end
    end)

    hook.Add("PlayerEndVoice", "SHRPCommsVoiceEnd", function(ply)
        if activeComm then
            ply:SetVoiceVolumeScale(1)
        end
    end)
